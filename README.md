...
тесты:
Artist
Вход: ID=1, Name="The Beatles"
Проверка: Корректность создания, метод ToString()
Выход: Объект Artist с правильными свойствами

Track
Вход: ID=1, Title="Song", Duration=300000 (5 мин)
Проверка: Форматирование длительности в ToString()
Выход: "Длительность: 05:00"

Тесты CRUD операций
Добавление
Вход: Artist(100, "New Artist")
Проверка: Уникальность ID, увеличение коллекции
Выход: Коллекция Artists.Count = 1

Удаление
Вход: artistId=1 (с альбомами)
Проверка: Ссылочная целостность
Выход: Сообщение об ошибке, данные не удалены

Тесты LINQ-запросов
Запрос 1: Треки > 5 мин
Вход: 3 трека: 2 мин, 5 мин, 6 мин
Проверка: Фильтрация >300000 мс
Выход: 1 трек (6 мин)

Запрос 2: Альбомы с исполнителями
Вход: Artist(1,"A"), Album(1,"X",1)
Проверка: JOIN ArtistId, сортировка
Выход: ["X - A"]

Запрос 3: Детали треков
Вход: Все 4 таблицы со связями
Проверка: Многотабличный JOIN, форматирование
Выход: "Трек | Альбом: X | Исполнитель: Y | Жанр: Z"

Запрос 4: Стоимость Blues
Вход: 2 трека в жанре Blues (100+200 руб)
Проверка: Поиск жанра, суммирование
Выход: 300.00 руб

Интеграционные тесты
Сохранение/загрузка Excel
Вход: Полный набор тестовых данных
Проверка: Цикл SaveToExcel → LoadFromExcel
Выход: Данные идентичны исходным

Ошибка загрузки
Вход: Путь "nonexistent.xlsx"
Проверка: Обработка отсутствия файла
Выход: Информативное сообщение, пустые коллекции

Граничные условия
Минимальные значения
Вход: Track с Duration=0, Size=0, Cost=0
Проверка: Корректность хранения нулевых значений
Выход: Данные сохраняются корректно

Максимальные значения
Вход: Track с int.MaxValue, long.MaxValue
Проверка: Обработка предельных значений
Выход: Данные не теряются
